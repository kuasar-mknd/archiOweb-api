...
export const startWebSocketServer = (port = 3001) => {
  // Sentinel: Set maxPayload to 50KB to prevent DoS via memory exhaustion
  const wss = new WebSocketServer({ port, maxPayload: 50 * 1024 })
  ...
  wss.on('connection', function connection (ws) {
    ...
    ws.on('error', (err) => {
      // Sentinel: Ignore Max payload size exceeded error as it's expected behavior for attacks
